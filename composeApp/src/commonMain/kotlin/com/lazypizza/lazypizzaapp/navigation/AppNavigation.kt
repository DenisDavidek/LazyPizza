...
import com.lazypizza.lazypizzaapp.features.product_catalog.presentation.MainProductCatalogScreen
...
fun AppNavigation(navHostController: NavHostController, modifier: Modifier){
    NavHost(navController = navHostController, startDestination = LazyPizzaScreen.MainProductCatalog, modifier = modifier){
        composable<LazyPizzaScreen.MainProductCatalog>{
            MainProductCatalogScreen(onNavigateToProductDetail = {
                navHostController.navigate(LazyPizzaScreen.ProductDetail)
            })
        }
        composable<LazyPizzaScreen.ProductDetail>{
            ProductDetailScreen(onClick = {})
        }
    }
}