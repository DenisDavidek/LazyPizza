...
object PhoneValidation {
    ...
    // Slovakia format: +421 followed by 9 digits
    // Accepts: +421 123 456 789, +421123456789, +421-123-456-789
    private val SK_PATTERN = Regex("^\\+?421\\s?([0-9]{3})\\s?([0-9]{3})\\s?([0-9]{3})$")
    ...
    private val SK_STRICT = Regex("^421[0-9]{9}$")
    private val UZ_STRICT = Regex("^998[0-9]{9}$")

    /**
     * Validates phone number for US (+1), Slovakia (+421), and Uzbekistan (+998) formats
    ...
    fun isValid(phone: String): Boolean {
        ...
        return when {
            ...
            cleaned.startsWith("421") -> SK_STRICT.matches(cleaned)
     ...
     * @return Country code ("1", "421", or "998") if valid, null otherwise
     */
    fun getCountryCode(phone: String): String? {
        val cleaned = phone.replace(Regex("[\\s()-]"), "").removePrefix("+")
        return when {
            cleaned.startsWith("1") && US_STRICT.matches(cleaned) -> "1"
            cleaned.startsWith("421") && SK_STRICT.matches(cleaned) -> "421"
    ...
    fun isComplete(phone: String): Boolean {
        ...
        return when {
            ...
            digits.startsWith("421") -> digits.length == 12 // 421 + 9 digits
    ...
    /**
     * Gets the expected total length for a phone number based on its country code
     */
    fun getExpectedLength(phone: String): Int? {
        val digits = extractDigits(phone)
        return when {
            digits.startsWith("1") -> 11
            digits.startsWith("421") -> 12
            digits.startsWith("998") -> 12
            else -> null
        }
    }
}